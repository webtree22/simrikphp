...
if($subcategory_of == $old_so)
{
	$cdo = $old_do;
}
else
{
	$q = mysqli_query($dbc, "select sn from category where subcategory_of=$subcategory_of");
	$n = mysqli_num_rows($q);
	$n = $n+1;
	$cdo = $n;
}

$q=mysqli_query($dbc,"select count(sn) as totalrec from category where subcategory_of=$subcategory_of");
